...
def _records_from_dataframe(df: pd.DataFrame | None, limit: int | None = None):
...
def _dict_from_dataframe(df: pd.DataFrame | None):
    if df is None or df.empty:
        return {}
...
def _extract_recommendation_mix(df: pd.DataFrame | None):
    records = _records_from_dataframe(df, limit=4)
    return next((record for record in records if record.get("period") == "0m"), records[0] if records else None)


def _extract_earnings_estimates(df: pd.DataFrame | None):
    if df is None or df.empty:
        return {}
...
def build_yahoo_overview(yf_ticker, info: dict):
    ...
    analyst_targets = yf_ticker.get_analyst_price_targets() or {}
    ...
    target_upside = (
        (target_reference / current_price) - 1
        if target_reference is not None and current_price not in (None, 0)
        else None
    )
    ...
    return {
        ...
        "market": {
            ...
            "dayChangePercent": (
                ...
                if current_price is not None and info.get("previousClose") not in (None, 0)
                else None
            ),
